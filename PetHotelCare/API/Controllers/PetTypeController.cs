...
namespace PetHotelCare.API.Controllers
{
    ...
    [ApiController]
    public class PetTypeController : CrudController<PetTypeRequest, PetTypeModel, PetType>
    {
        private readonly ApplicationDbContext _context;
        public PetTypeController(ApplicationDbContext context, IMapper mapper) : base(context, mapper)
        {
            _context = context;
        }
        ...
        public async Task<PaginationModel<PetTypeModel>> GetAsync(int page = 1, string? query = "")
        {
            ...
            var entities = enumerable.Skip((page - 1) * 10)
                .Take(10)
                .ToList();
            var models = entities.Adapt<List<PetTypeModel>>();
            return new PaginationModel<PetTypeModel>(models, enumerable.Count());
        }
    }
}
