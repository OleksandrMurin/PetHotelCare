...
const EditPetDialog = ({ open, onClose, onSave, pet, petTypes, tags, avatarFile, setAvatarFile }) => {
    ...
    useEffect(() => {
        if (pet) {
            setLocalPet({
                ...pet,
                prohibitedTags: Object.keys(pet.prohibitedTags).map(id => parseInt(id, 10)) 
            });
        }
    }, [pet]);

    // useEffect(() => {
    //     console.log('Pet received in EditPetDialog:', pet);
    //     console.log('Local Pet state:', localPet);
    // }, [pet, localPet]);
    ...
    return (
        <Dialog open={open} onClose={onClose}>
            ...
            <DialogContent>
                <DialogContentText>Update the information of your pet.</DialogContentText>
                <TextField
                    autoFocus
                    margin="dense"
                    label="Name"
